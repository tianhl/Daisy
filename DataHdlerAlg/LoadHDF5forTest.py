...
class LoadHDF5forTest(Daisy.Base.DaisyAlg):
    ...
    def execute(self, startswith, seperator, idx, output_dataobj):
        ...
        if not (filelist[0].split('.')[-1].upper() == 'H5'):
            self.LogError('Files should be h5')
            return False

        projs = np.zeros((len(filelist),1,2048),dtype=int)
        for i, filename in enumerate(filelist):
            nfile = self.ipath+'/'+filename
            with h5py.File(nfile, 'r') as hfile:
                hdata = hfile['data']
                projs[i][0] = hdata[0,idx,:]   
        self.data[output_dataobj] = projs
